...
export class DungeonDataFetcher extends DataFetcher<Dungeon> {
    ...
    mapServerDungeonToDungeon = (dungeon: ServerDungeon): Dungeon => {
        ...
        return { id, name, summary, levelMin: level_min, levelMax: level_max, playerCount: player_count }
    }
    ...
    getList = async (): Promise<Dungeon[]> => {
        ...
        return json.map(serverDungeon => this.mapServerDungeonToDungeon(serverDungeon));
    }

    addSingle = async (dungeon: AddDungeon): Promise<{ entity: Dungeon | undefined; httpCode: number }> => {
